...
@celery_app.task()
def subfinder_task(scan_id: str, target_id: str, host: str):
    ...
    try:
        ...
        publish_event(scan_id, "tool_started", "subfinder", 10)

        nmap_output = run_subfinder(host)
        save_result(db, scan_id, tool="subfinder", raw=nmap_output)

        scan.status = "completed"
        db.commit()

        publish_event(scan_id, "tool_completed", "subfinder", 70)
...
def run_subfinder(host: str) -> str:
